...
const Chart = ({ dataPoints, extremes }) => {
  ...
  useEffect(() => {
    if (window.screen.width < vb_width) {
      setScaleFactor((window.screen.width - 16)/vb_width)
    }
  }, []);

  return (
    <svg width={vb_width * scaleFactor} height={vb_height * scaleFactor} viewBox={`${vb_x} ${vb_y} ${vb_width} ${vb_height}`}>
